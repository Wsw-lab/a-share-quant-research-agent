...
from a_share_quant_agent.historical_assets import CAPITAL_FLOW_FIELDS, discover_data_assets, write_asset_inventory
...
EXTRACT_DIR = "investoday_capital_flow_factors"
RUNS_DIR = "runs"
ENDPOINT = "stock/daily-fund-flows"
OUTPUT_COLUMNS = ("date", "symbol", "stockCode", "stockName", *CAPITAL_FLOW_FIELDS, "capitalFlowSource")


def main() -> int:
    args = _parse_args()
    try:
        result = extract_investoday_capital_flow_factors(
    ...
    print(f"Main flow coverage: {float(coverage.get('mainNetInflow', 0.0) or 0.0):.2%}")
...
def extract_investoday_capital_flow_factors(
    *,
    ...
    shard_dir = run_dir / "capital_flow_shards"
    ...
    if not selected_symbols:
        raise DataSourceError("No symbols were selected for Investoday capital-flow extraction.")
    ...
    result: dict[str, object] = {
        ...
        "status": "running",
        "decision": "extract_strict_lagged_investoday_daily_fund_flows",
        "endpoint": ENDPOINT,
    ...
    raw, errors = _fetch_capital_flow_records(
        selected_symbols,
        start=start,
        end=end,
        shard_dir=shard_dir,
    ...
    if errors:
        ...
        result["decision"] = "retry_failed_investoday_capital_flow_batches"
        result["blockers"] = [f"Investoday capital-flow extraction had {len(errors)} failed batches."]
    ...
    flows = _normalize_capital_flow_records(raw, end=end)
    if flows.empty:
        raise DataSourceError("Investoday daily fund flow endpoint returned no usable records.")

    output_path = asset_root_path / "market" / "daily_fund_flows.csv" if execute else run_dir / "daily_fund_flows.csv"
    if execute:
        result["backup"] = _backup_existing_capital_flow_asset(asset_root_path, extract_id)
    ...
    flow_metrics = validation.get("daily_fund_flow_metrics") if isinstance(validation.get("daily_fund_flow_metrics"), dict) else {}
    field_coverage = flow_metrics.get("field_coverage") if isinstance(flow_metrics.get("field_coverage"), dict) else {}
    ...
    result.update(
        {
            "status": status,
            "decision": "run_capital_flow_strategy_factory" if execute else "review_dry_run_then_execute",
            "rows": int(len(flows)),
            "symbols_covered": int(flows["symbol"].nunique()),
            "raw_rows": int(len(raw)),
            "raw_columns": [str(column) for column in raw.columns],
            "data_hash": dataframe_hash(flows),
            "output_csv": str(output_path),
            "production_ready": bool(validation.get("production_data_ready", False)),
            "validation": validation,
            "inventory": inventory,
            "coverage": {
                "mainNetInflow": float(field_coverage.get("mainNetInflow", 0.0) or 0.0),
                "netInflowLarge": float(field_coverage.get("netInflowLarge", 0.0) or 0.0),
                "netInflowXlarge": float(field_coverage.get("netInflowXlarge", 0.0) or 0.0),
            },
            "blockers": [
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extract Investoday daily fund flows into canonical daily_fund_flows.csv.")
    ...
    parser.add_argument("--execute", action="store_true", help="Write data_assets/market/daily_fund_flows.csv.")
...
def _fetch_capital_flow_records(
    symbols: tuple[str, ...],
    *,
    start: str,
    end: str,
    ...
    if workers == 1:
        for index, batch in batches:
            batch_index, path, error = _fetch_capital_flow_batch(
                index,
                batch,
                start=start,
                end=end,
...
def _print_batch_progress(
    ...
    *,
    ...
    if error:
        ...
        status = "empty"
    print(
        f"[capital-flow] batch {batch_index}/{total_batches} {status} elapsed={elapsed:.1f}s",
        file=sys.stderr,
        flush=True,
    )
...
def _normalize_capital_flow_records(raw: pd.DataFrame, *, end: str) -> pd.DataFrame:
    if raw.empty:
        return pd.DataFrame(columns=OUTPUT_COLUMNS)
    frame = raw.copy()
    if "stockCode" not in frame and "symbol" not in frame:
        raise DataSourceError("Investoday capital-flow payload is missing stockCode/symbol.")
    if "date" not in frame:
        raise DataSourceError("Investoday capital-flow payload is missing date.")
    if "symbol" in frame:
    ...
    if "stockName" not in frame:
        frame["stockName"] = ""
    frame["capitalFlowSource"] = f"investoday:{ENDPOINT}"
...
def _build_staging_validation_root(*, staging_root: Path, asset_root: Path, flow_path: Path) -> Path:
    ...
    flow_target = staging_root / "market" / "daily_fund_flows.csv"
    flow_target.parent.mkdir(parents=True, exist_ok=True)
    if flow_target.exists() or flow_target.is_symlink():
        flow_target.unlink()
    _link_or_copy(flow_path, flow_target)
    return staging_root


def _backup_existing_capital_flow_asset(asset_root: Path, extract_id: str) -> dict[str, object]:
    source = asset_root / "market" / "daily_fund_flows.csv"
    if not source.exists():
        return {"status": "skipped", "reason": "No existing daily_fund_flows.csv to back up."}
    backup_root = asset_root / "backups" / extract_id
    backup_path = backup_root / "market" / "daily_fund_flows.csv"
...
def _write_extract_artifacts(reports_root: Path, result: dict[str, object]) -> dict[str, Path]:
    ...
    json_path = run_dir / "capital_flow_extract.json"
    md_path = run_dir / "capital_flow_extract.md"
    latest_json = extract_dir / "latest_capital_flow.json"
    latest_md = extract_dir / "latest_capital_flow.md"
...
def _render_extract_markdown(result: dict[str, object]) -> str:
    ...
    coverage = result.get("coverage") if isinstance(result.get("coverage"), dict) else {}
    lines = [
        "## Investoday Capital Flow Factors",
        "",
        ...
        f"- Main net inflow coverage: {_format_rate(coverage.get('mainNetInflow', 0.0))}",
        f"- Large/xlarge net inflow coverage: {_format_rate(coverage.get('netInflowLarge', 0.0))} / {_format_rate(coverage.get('netInflowXlarge', 0.0))}",
        ...
        (
            ...
            "`list[dict]` with `stockCode`, `date`, `mainNetInflow`, `netInflowLarge`, `netInflowXlarge` |"
        ),
        "",
        "### PIT Usage",
        "",
        "- Strategy date T uses only the latest daily fund-flow row strictly before T.",
        "- Same-day flow rows are excluded by the production loader to avoid after-close leakage.",
        "",
        "### Paths",
        "",
        "| Name | Path |",
...
def _make_extract_id(now: datetime | None = None) -> str:
    ...
    return f"investoday_capital_flow_{stamp}"
