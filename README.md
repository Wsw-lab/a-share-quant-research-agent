...
- `a_share_quant_agent.confirmatory_study` 的 `run` / `verify` / `run-stage2` / `verify-stage2` / `status` 命令；其中 Stage-2 命令只覆盖已实现的 IC core，并在缺少完整注册链或授权输入时 fail closed。`verify-stage2 --receipt ...` 可独立验证公开 receipt；显式传入 `--authorization-consumption ...` 可核对私有授权消费记录，显式传入 `--endpoint-ledger ...` 才审计不公开的逐证券 endpoint ledger；
