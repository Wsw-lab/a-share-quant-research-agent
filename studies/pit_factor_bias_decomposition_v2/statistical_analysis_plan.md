...
The legacy repository record describes the pilot input as a licensed local export covering 3 January 2023 through 24 July 2026 and evaluates a repository-locked test window of 18 monthly cross-sections from January 2025 through June 2026. The new Stage-2 rights packet has not re-attested the public basis for the existing pilot hashes, aggregates, or controlled-review access; continued hosting and reuse therefore remain subject to a separate licensor or administrator review. The legacy record reports all 16 combinations of four factors and four cumulative variants. Publication-date alignment produces a negative signed shift in the pilot ROE and composite rank information coefficients; the bundled implementation variant makes both means negative. Momentum and low-volatility ICs disagree in sign with their top-quintile-minus-universe contrasts.
...
Only outcome-blind integrity and coverage evidence may be examined before registration. Automated custodial validators may parse raw numeric bytes to check schema, finiteness, dates, identifiers, exact endpoint presence, and permitted aggregate coverage, but neither the validators nor a human reviewer may compute, retain, release, or inspect factor returns, signal ranks, ICs, portfolio outcomes, test statistics, or variant orderings. The coverage report must not contain those research outcomes.
...
2. The quote audit shows at least 15 distinct quote dates that belong to the bound official calendar in every target month; repeated security rows cannot inflate the session count. Point-in-time monthly coverage is constructed from strict SH/SZ A-shares satisfying `listDate <= signal date <= delistDate`, with a missing `delistDate` treated as open-ended. A0 uses a separate fixed terminal-survivor comparator: at each historical signal session the security must already be listed, and its `delistDate` must be null or strictly after **2023-01-31**, the fixed required quote/outcome cutoff. Extraction-current `listStatus` and acquisition date never move this terminal boundary. For every target month, every potential candidate in `active strict A-share master ∩ signal-session quote identifiers` must have exact rows at `t`, `t+1`, `t+20`, and `t+21`; `exact_endpoint_symbol_count` must equal `signal_session_candidate_symbol_count`. Separately, for every one of the 156 first-session rebalances, at least 1,000 such identifiers must jointly have quote rows at every official session from `t-60` through `t` for the implemented momentum history, at every official session from `t-20` through `t` for 20 daily valuation returns, at every session from `t-19` through `t` for the 20-session amount mean, and at all four exact endpoints. This pre-lock check uses only identifier/date presence: a dense rebalance date, an aggregate monthly symbol count, or file-level minimum and maximum dates cannot establish per-security warm-up and endpoint availability.
...
5. Every quote row carries `close_observation_type`, exactly `traded_close` when `is_suspended=false` and exactly `suspension_valuation` when `is_suspended=true`. Both types must occur among active-master signal-session candidates. Provider documentation and delivery evidence must prove that each suspension valuation was recorded or published by the supplier for that exact official session; research code never constructs one by forward-filling a last price. Corporate-action handling is documented and hash-bound. The resulting returns and rank ICs are valuation diagnostics, not executable trade returns. Unadjusted open and nonfill semantics are required only if the planned portfolio module is later implemented and frozen.
6. The canonical daily `amount` is in exact CNY before the raw quote file is hash-bound, with `amount_unit=CNY` on every row. The provider-native unit, conversion rule, cutoff timing, and conversion-provenance hash are independently reviewed; missing, differently cased, whitespace-padded, thousand-CNY, or other unit tokens block the study. ST and suspension status are available under documented timing semantics.
7. Data rights permit the intended local analysis, the public release of aggregate results, field definitions, and file hashes, and publication of the exact official-calendar session dates embedded in the receipt. Each Conditional restriction has a unique identifier and exactly one satisfied-condition review linked by that identifier and the same permission field; missing, duplicate, unknown, permission-mismatched, or false-permission mappings fail closed.
...
11. Exact per-symbol endpoint semantics are reviewed. The current IC adapter supports only an exact provider-recorded traded close or an exact supplier-recorded/published same-session suspension valuation on the required official session. Researcher-generated last-price carry-forward is forbidden, and the delisting or terminal-wealth adapter remains unimplemented. Rights permit a private per-security endpoint-reason ledger to be retained and hash-bound and aggregate endpoint reason counts to be published.

Column names do not satisfy gates 5-7. An authorized reviewer must complete a hash-bound attestation that separately verifies provider close-observation semantics, the provider-native amount unit and normalization into CNY before input binding, supplier-recorded same-session suspension valuations, non-degenerate tradability fields, rights to publish aggregate outputs, and rights to embed the exact official-calendar session dates in a receipt. Without that attestation, the coverage tool reports `BLOCKED` even when every required column is present.
...
The target population is ordinary China A-shares listed on the Shanghai or Shenzhen exchanges. B-shares, funds, bonds, preferred shares, and non-equity instruments are excluded using the historically valid security type. A security enters the point-in-time universe on or after its listing date and exits after its delisting date. The deliberately biased A0 comparator uses the fixed terminal date 31 January 2023: after requiring that a security was already listed on the historical signal session, it retains only a null `delistDate` or one strictly after that cutoff. This rule is independent of when the files are acquired or which status the provider reports at extraction. Active rows must have null delist dates, and delisted rows must have valid delist dates on or after listing.

All three symbol-bearing inputs use the fixed identifier contract token `provider_stable_exchange_qualified_security_identifier_with_reviewed_code_change_mapping_v1`. Exact exchange-qualified formatting is only a mechanical gate. Before binding the inputs, a human reviewer must hash-bind the provider's stable-identifier definition and a complete mapping for historical code changes or reassignments, and must attest that the same identity semantics govern quotes, stock master, and fundamentals. This requirement does not imply a revision- or vintage-data claim.
...
- **Momentum 60d:** provider close observation at the signal date divided by the provider close observation 60 sessions earlier, minus one; any suspended-session input must be an attested same-session supplier valuation.
...
The current publication-date variants require `publishDate < signal date`: a same-day release is never used in that session's close signal. The current runner and protocol do not support an intraday timestamp substitution. Any future timestamp-and-timezone rule would require a separately implemented and tested adapter plus an externally timestamped amendment, full plan/manifest re-freeze, fresh external registration, and fresh execution authorization, all before any Stage-2 outcome access; it cannot replace this rule within the present registered design.
...
The IC clock is a valuation diagnostic, not a simulated execution claim. The no-lag outcome is the provider close observation at session `t+20` divided by that at signal session `t`, minus one. The lag outcome is the provider close observation at `t+21` divided by that at `t+1`, minus one. On a traded row the observation is `traded_close`; on a suspended row it is a `suspension_valuation` recorded or published by the supplier for that exact session. The research code never forward-fills one. The signal-eligible denominator is fixed before outcome lookup. A missing required symbol endpoint makes the entire factor-variant-month cell non-estimable and the study `INSUFFICIENT_EVIDENCE`; the security may not be dropped, advanced to its next observed quote or reopening date, valued using a researcher-created last price, or assigned zero or any other default recovery. A verified delisting or terminal-wealth adapter is not implemented and cannot be improvised after outcome access. The exact calendar bytes, schema version, source provenance, timezone, date range, and SHA-256 hash are design inputs and must be bound by the frozen plan core and design manifest, but calendar position alone does not prove that a per-security endpoint close observation exists.
...
- **A0:** fixed 31 January 2023 terminal-survivor universe (`delistDate` null or strictly after the cutoff, conditional on listing by the signal session) and report-period availability;
...
3. require 20-session mean amount of at least CNY 5 million, computed only from canonical rows whose exact `amount_unit` is `CNY`;
...
Signals would be formed after close `t`. Baseline entry would use the unadjusted open at `t+1` and exit the unadjusted open at `t+21`; a lag component would shift those endpoints to `t+2` and `t+22`. No nonfill, suspension-valuation execution, delisting, terminal-wealth, last-price, or default-recovery rule is authorized for this planned portfolio protocol. The IC core's supplier-recorded suspension valuation is not an executable fill. Any future portfolio module must acquire the required execution and terminal-event data, implement and test a complete fail-closed rule, and freeze it under a separate registration before viewing its outcomes.
...
- equal-weight top-quintile minus universe mean return for continuity with the pilot. Signals use average percentile ranks; the bottom group is `rank <= 0.2` and the top group is `rank > 0.8`. Equal signal values receive the same average rank and stay together, so either group may differ from 20% when a tie spans a cutoff. With 1,000 distinct values, each tail contains exactly 200 observations;
...
The monthly candidate set is the intersection of the lifecycle-eligible strict SH/SZ A-share master and securities with an exact provider close observation on the signal session; suspended candidates must be represented by an attested same-session `suspension_valuation`, not a carried or later price. Before design freeze, every candidate must have all four exact endpoint rows, so the endpoint count equals the candidate count in every target month. The pre-lock coverage report separately discloses aggregate active-master, signal-session-candidate, exact-endpoint, and complete joint-contract counts and also requires at least 1,000 complete identifiers in every target month. Missing signals are not filled with cross-sectional means. A candidate with a missing required factor field is excluded from that factor's signal-eligible cross-section, and aggregate factor-specific missing counts are reported. The current runner does not attribute those signal omissions to a more granular reason taxonomy. The composite requires all three primary component ranks. The signal-eligible denominator is finalized before outcome lookup, and outcome availability cannot remove a security from it.

Every signal-eligible record receives a machine-readable endpoint-resolution code. The resolved path covers an exact provider-recorded traded close or attested same-session suspension valuation; the fixed unresolved codes are `MISSING_EXACT_FORWARD_EXIT`, `MISSING_EXACT_LAG_ENTRY`, and `MISSING_EXACT_LAG_EXIT`. The current adapter records which required close observation is absent but does not infer whether delisting or another economic cause produced that absence. Any non-finite valuation return after exact endpoints resolve is a separate cell-integrity failure, not an endpoint reason code. The result receipt exposes aggregate code counts and a SHA-256 for the complete private ledger, and its verifier requires the ledger-key set to equal the signal-eligible key set. This endpoint audit does not claim a full per-security explanation of all signal or universe exclusions. Security identifiers must be stable across symbol changes or linked through a documented permanent identifier. Duplicate date-symbol quote keys and duplicate effective security-master records fail closed.
...
The target remains the full 18-variant, 72-cell lattice. If the supplier cannot prove exact-session provenance for suspension valuations, Stage 2 remains blocked. The only allowed fallback is a prospective amendment before external registration and before any Stage-2 outcome access: delete all eight factorial variants containing the suspension component, leaving the two timing/universe comparators plus eight suspension-off factorial variants (10 variants and 40 factor–variant cells), then re-enumerate the estimands and multiplicity family and externally timestamp and re-freeze the SAP, plan, manifest, and authorization. That fallback is forbidden after registration or outcome access.
...
- fixed logical file labels and permitted SHA-256 hashes, plus only the aggregate row and date counts carried in explicit authorized receipt fields; raw/control byte sizes, private operator basenames, and local paths are excluded, and every disclosed item remains conditional on written rights;
- the official common-session calendar's schema, source provenance, permitted aggregate row and date counts, and SHA-256 hash; its raw byte size remains private;
...
After the one-use execution authorization is consumed, the runner captures each
of the four raw inputs exactly once as bytes. The authoritative coverage
recomputation, panel loaders, and public hash evidence all use those same
captured bytes. A later path replacement therefore cannot substitute a
different analysis panel; a captured-byte hash mismatch fails after consuming
the authorization and produces no result receipt.
...
1. Retain `coverage_probe_spec.v1.json` byte-for-byte; complete the outcome-blind prior-specification inventory against an already-existing audited base commit, including preparer identity, timezone-aware chronology, canonical entries hash, and `manifest_eligible_outcome_blind` status; commit those exact inventory bytes and `coverage_probe_spec.v2.json` in a descendant commit; create a canonical package manifest binding both hashes and that containing Agent commit; externally timestamp it after inventory generation and independently verify it before the bounded exact-route probe begins; and then freeze the v2 probe receipt, outcome-blind coverage report, rights/semantics review, official calendar, the same immutable inventory bytes, and prior-exposure attestation. The probe rights review binds the exact timestamp-proof and rights-review bytes. A finite term additionally requires hash-evidenced survival of private probe-artifact retention and continued availability of the already-published aggregate receipt and metadata after expiry; an affirmatively non-expiring term carries no contradictory post-expiry claim. `repository_snapshot.head_commit` identifies the pre-inventory base and must be an ancestor of both the probe commit and later plan code commit; the exact inventory hash supplies downstream identity without a self-referential Git commit field.
...
5. Independently verify the receipt and create `execution_authorization`, which points backward to the manifest, receipt, frozen plan core, calendar, and gate artifacts and authorizes the registered runner's access to the sealed panel. Before that authorization, the custodian may run only the fixed structural/numeric validators described above; no Stage-2 factor-return, IC, rank, portfolio, test-statistic, or variant-result artifact may be computed, released, or human-inspected.
...
7. At execution, require explicit result and authorization-consumption directories and reject either directory if its nearest existing parent identifies it as inside any Git worktree, including an unrelated repository, linked checkout, QData checkout, or ignored path. Only then recompute the control-artifact hashes, cross-check the raw-input digests declared throughout the registered chain without opening those raw inputs, and atomically create the private authorization-consumption sidecar keyed by the authorization-file SHA-256. After that claim, read each raw input exactly once, authenticate its captured-byte digest against every registered declaration, and reuse those bytes for coverage recomputation, panel loading, and receipt evidence before computing any factor/outcome value. Exclusive creation makes a second local claim fail closed; an interrupted run or a captured-byte mismatch remains consumed, produces no result receipt, and requires a new authorization. The sidecar is outside the immutable authorization and final plan envelope, so it points backward without creating a hash cycle.

The current runner accepts only `runner_scope = ic_core_only` and only the explicit human-verification evidence types in the maintained templates. It does not implement cryptographic signature or registry-inclusion-proof validation, so a cryptographic label alone cannot pass. Hashes establish the identity and integrity of retained local artifacts but do not authenticate the provider's timestamp by themselves; a named authorized human must verify the retained provider page, email, or journal record and hash that evidence before probe execution starts. The validator enforces `timestamped <= verified <= execution start/first request`. Publication of artifact hash/name/size/row-count metadata and timestamp verifier identity/URI requires separate affirmative consent. This is the explicit human trust boundary. A future cryptographic path requires a separately implemented, tested, and frozen protocol.
...
- the private probe output, authoritative coverage-report target, explicit fresh Stage-2 output directory, or authorization-consumption directory resolves inside any Git worktree; the coverage-report target already exists; or the sidecar cannot be exclusively and durably created before outcome load, already exists for that authorization hash, or differs from the hash-bound record in the receipt;
...
- any quote row lacks `close_raw`, finite positive `adjustment_factor`, exact `price_adjustment_method=close_equals_close_raw_times_adjustment_factor`, or exact `price_adjustment_convention=provider_cumulative_backward_adjusted_hfq_no_rebasing`; `close != close_raw × adjustment_factor` beyond the frozen `1e-12` relative/absolute tolerances; or the human review lacks distinct evidence hashes for the provider raw-close definition, cumulative-factor convention, and normalization/adapter record. A later portfolio claim additionally stops if unadjusted execution prices cannot be distinguished from adjusted research prices;
