...
当前证据入口以 [README](../README.md) 为准：合成 demo 与严格 QData fixture 走唯一离线绿色路径；已观察的真实市场 pilot 另有绑定输入哈希、方案和代码 commit 的 receipt。已运行证据的公开状态只从这个通过验证的 receipt 派生，不从历史 readiness 或 registry 推断；其历史 `OOS` 标签只表示仓库内锁定测试窗。Stage-2 期刊研究保持 `BLOCKED_FOR_STAGE2`，不公开授权原始数据，也不声称拥有完整 revision/vintage 历史。
